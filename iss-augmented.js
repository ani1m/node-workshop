...
function coordinates() {
    ...
    prompt('location')
        .then(function(location) {
            return request("https://maps.googleapis.com/maps/api/geocode/json?address=" + location)
        })
        .then(function(googleResponse) {
            var parsedRes = JSON.parse(googleResponse)
            lat = parseFloat(parsedRes.results[0].geometry.location.lat)
            long = parseFloat(parsedRes.results[0].geometry.location.lng)
            return request("http://api.open-notify.org/iss-now.json")
        })
        .then(function(issResponse) {
            var searchResults = JSON.parse(issResponse)
            var long2 = parseFloat(searchResults.iss_position.longitude)
            var lat2 = parseFloat(searchResults.iss_position.latitude)
